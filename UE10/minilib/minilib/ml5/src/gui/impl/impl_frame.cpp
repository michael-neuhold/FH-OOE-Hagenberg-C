//       $Id: impl_frame.cpp 617 2020-01-09 21:44:18Z p20068 $
//      $URL: https://svn01.fh-hagenberg.at/se/minilib/ml5/trunk/product/ml5/src/gui/impl/impl_frame.cpp $
// $Revision: 617 $
//     $Date: 2020-01-09 22:44:18 +0100 (Do., 09 Jän 2020) $
...
void frame::add_menu (std::string const & title, mitem_cont_t const & items) {
   if (menu_not_empty (title, items)) {   // protect menu.Append
      ...
      for (auto const & [t, h] : items) {
         ...
         m_menu_item_map[m_item_id++] = {util::erase_all (title, '&'), util::erase_all (t, '&')};
      }
...
bool frame::menu_not_empty (std::string const & title, mitem_cont_t const & items) {
   if (!std::empty (title) && !std::empty (items)) {
      auto const & [t, h] {*std::begin (items)}; return !std::empty (t);
   }
...
wxMenu & frame::new_menu_about () {
   ...
   Bind (wxEVT_MENU, [] (wxCommandEvent const &) {
      ...
      wxMessageBox ("blah ...", "About MiniLib "s + version_string, wxOK | wxICON_INFORMATION);
   }, wxID_ABOUT);
